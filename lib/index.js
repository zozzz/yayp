...
class Parser {
    ...
    isDocumentSeparator(offset) {
        ...
        if ((ch === 46 /* DOT */ || ch === 45 /* DASH */) && this.data.charCodeAt(offset + 1) === ch && this.data.charCodeAt(offset + 2) === ch && isWSorEOF(this.data.charCodeAt(offset + 3))) {
...
function loadAll(data, options = {}) {
    options.allowMultipleDocuments = true;
    return _load(data, options);
}
function _load(data, options) {
    let loader = new Loader(options.document || YamlDocument, options);
    let documents = loader.load(data, options.filename);
    loader.dispose();
